...
def get_new_title_and_edit_segments(word_timestamps: list) -> tuple[str | None, list]:
    ...
    prompt = (
        ...
        'Identify and exclude the following segments:\n'
        ' - weather segments, including the interval of silence afterwards\n'
        ' - the "Fidelity Financial Minute", if present'
        ' - the "World Focus" segment where the anchor talks about news around the world\n'
        ' - the "This day in history" segment where the anchor talks about news from this day in history\n'
        ' - duplicate sports segments\n'
        ' - commercials and sponsored segments\n'
        ' - every preview/rundown of an upcoming segment, with the exception of the very opening rundown; '
            'also exclude the 7:30 rundown\n'
        ' - the 30-or-so second call-to-action for Youtube, if present (i.e. the part including "Don\'t '
            'forget to like and subscribe")\n'
        '\n'
        'Remember to always start at timestamp 0.00.\n'
        '\n'
        'In addition, generate an new title for the edited podcast. The title should briefly summarize '
        'the three most important stories and be no longer than 55 characters. Format the title as '
        '"First Topic, Second Topic, Third Topic", always using APA title case.  Return a JSON object '
        'representing the new title and the valid content segments to KEEP. All timestamps should be '
